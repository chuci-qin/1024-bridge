...
# ============================================
# 启动组件（使用编译好的二进制文件直接运行，输出同时写入日志和 stdout）
# ============================================
log "Starting relayer components..."

# 启动 s2e
cd "$APP_DIR/s2e"
log "Starting s2e from $(pwd)..."
"$S2E_BIN" 2>&1 | tee "$LOG_DIR/s2e.log" | sed 's/^/[s2e] /' &
S2E_PID=$!
log "s2e started (PID: $S2E_PID)"

# 启动 e2s-listener
cd "$APP_DIR/e2s-listener"
log "Starting e2s-listener from $(pwd)..."
"$E2S_LISTENER_BIN" 2>&1 | tee "$LOG_DIR/e2s-listener.log" | sed 's/^/[e2s-listener] /' &
E2S_LISTENER_PID=$!
log "e2s-listener started (PID: $E2S_LISTENER_PID)"

# 启动 e2s-submitter
cd "$APP_DIR/e2s-submitter"
log "Starting e2s-submitter from $(pwd)..."
"$E2S_SUBMITTER_BIN" 2>&1 | tee "$LOG_DIR/e2s-submitter.log" | sed 's/^/[e2s-submitter] /' &
E2S_SUBMITTER_PID=$!
