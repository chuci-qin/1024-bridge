...
//!
//! 记录每条链的扫描进度，避免重启后重复扫描：
//! - EVM 链：记录上次扫描到的区块号
//! - SVM 链：记录上次扫描到的最新交易签名（signature）
//!
//! 文件路径格式：{checkpoints_dir}/{direction}_{chain_id}.json
//! 例如：/data/checkpoints/inbound_1.json（Ethereum Mainnet 的入站 checkpoint）
//!
//! 保存使用原子写入（先写 .tmp 再 rename），避免写入中途崩溃导致文件损坏。
...
/// 构造 checkpoint 文件路径：{dir}/{direction}_{chain_id}.json
