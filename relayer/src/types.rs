...
impl BridgeEventData {
    /// Borsh 序列化后的固定长度：4 个 bytes32 + 6 个 u64 = 4×32 + 6×8 = 176 字节
    pub const BORSH_LEN: usize = 32 * 4 + 8 * 6;
}
...
impl fmt::Display for BridgeEventData {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            f,
            "BridgeEvent(nonce={}, raw_amount={}, amount={}, src_chain={}, dst_chain={})",
            self.nonce, self.raw_amount, self.amount, self.source_chain_id, self.target_chain_id,
        )
    }
}
...
mod tests {
    ...
    fn borsh_len_constant_matches_serialization() {
        ...
        assert_eq!(bytes.len(), BridgeEventData::BORSH_LEN);
        // 4 个 bytes32 + 6 个 u64 = 128 + 48 = 176
        assert_eq!(BridgeEventData::BORSH_LEN, 176);
    }
    ...
    fn borsh_roundtrip_preserves_all_fields() {
        ...
        let recovered = BridgeEventData::try_from_slice(&bytes).expect("borsh deserialize");
