...
/// Start the event processor loop (reads from queue written by sol2svm-listener).
pub async fn start_processor(config: SubmitterConfig) -> Result<()> {
    info!("Starting sol2svm event processor");
                    ...
                    match serde_json::from_str::<StakeEventData>(&content) {
                        Ok(event) => {
                            info!(
                                ...
                                "Processing Solana->1024chain event"
                            );
...
async fn submit_signature(
    ...
    // Derive PDA accounts on 1024chain
    ...
    info!(nonce = event.nonce, "Submitting transaction to 1024chain");
...
fn create_submit_signature_instruction(
    ...
    // submit_signature Anchor discriminator
    let discriminator: [u8; 8] = [205, 224, 80, 14, 239, 119, 52, 129];

    let mut data = Vec::new();
    data.extend_from_slice(&discriminator);
    event.nonce.serialize(&mut data)?; // param 1: nonce

    // param 2: event_data (StakeEventData fields in order)
    event.nonce.serialize(&mut data)?;
    event.amount.serialize(&mut data)?;
    event.block_height.serialize(&mut data)?;
    event.sender.serialize(&mut data)?;         // 32 bytes (Solana pubkey for sol2svm)
    event.receiver_pubkey.serialize(&mut data)?;

    signature.to_vec().serialize(&mut data)?;   // param 3: signature (Vec<u8>)
...
mod tests {
    ...
    #[test]
    fn test_to_compact_solana_sender() {
        ...
        let compact = event.to_compact().expect("Should convert Solana sender");
        assert_eq!(compact.sender.len(), 32);
        // Solana pubkey fills all 32 bytes (no zero-padding prefix)
        assert!(compact.sender[0] != 0 || compact.sender[1] != 0);
