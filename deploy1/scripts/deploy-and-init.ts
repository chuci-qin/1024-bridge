 ...
 * deploy-and-init.ts
 *
 * Initialize and configure a deployed Bridge1024 program (unified contract).
 * Works for both Solana and 1024chain deployments — the only difference is
 * BRIDGE_FEE (0 for Solana, >0 for 1024chain) and the RPC URL / chain IDs.
 *
 * Required environment variables:
 *   ADMIN_KEYPAIR_PATH - Path to admin keypair JSON file
 *   PROGRAM_ID         - Deployed program ID (base58)
 *   RPC_URL            - RPC endpoint (fallback: SVM_RPC_URL for backward compat)
 *   USDC_MINT          - USDC SPL token mint address (base58)
 *   PEER_CONTRACT      - Peer program ID on the other chain (base58 or hex)
 *   SOURCE_CHAIN_ID    - This chain's ID
 *   TARGET_CHAIN_ID    - Peer chain's ID
...
const RPC_URL: string = process.env.RPC_URL || process.env.SVM_RPC_URL || (() => { throw new Error("Missing required env: RPC_URL (or SVM_RPC_URL)"); })();
...
async function main() {
  console.log("============================================");
  console.log("  Bridge1024 - Initialize & Configure");
  console.log("============================================");
  console.log(`Program ID:     ${PROGRAM_ID}`);
  console.log(`RPC URL:        ${RPC_URL}`);
  console.log(`USDC Mint:      ${USDC_MINT}`);
  console.log(`Peer:           ${PEER_CONTRACT}`);
...
main()
  .then(() => process.exit(0))
  .catch((err) => {
    console.error("Bridge deployment failed:", err);
    process.exit(1);
  });
