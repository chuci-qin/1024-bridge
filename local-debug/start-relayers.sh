#!/usr/bin/env bash
# start-relayers.sh
# Generate .env files and start 3 relayers (9 processes) locally
set -euo pipefail
...
log() { echo "[relayer][$(date '+%H:%M:%S')] $*"; }
die() { echo "[relayer] ERROR: $*" >&2; exit 1; }

[ -f "$DEPLOYMENT_FILE" ] || die "deployment.json not found. Run deploy-all.sh first."
...
log "=========================================="
log "  Starting 3 Relayers (9 processes)"
log "=========================================="
log "EVM Contract: $EVM_CONTRACT"
log "SVM Program:  $SVM_PROGRAM"
log ""

# Port scheme: relayer N -> s2e=80N1, e2s-submitter=80N2, e2s-listener=80N3
# N=0 -> 8081,8082,8083  N=1 -> 8181,8182,8183  N=2 -> 8281,8282,8283
RELAYER_ENVS=(
  "$PROJECT_ROOT/relayer/easypanel-envs/arbsep-relayer1.env"
  "$PROJECT_ROOT/relayer/easypanel-envs/arbsep-relayer2.env"
  "$PROJECT_ROOT/relayer/easypanel-envs/arbsep-relayer3.env"
)
...
for i in 0 1 2; do
  N=$((i + 1))
  S2E_PORT=$((8081 + i * 100))
  SUBMITTER_PORT=$((8082 + i * 100))
  LISTENER_PORT=$((8083 + i * 100))
...
EOF
  ...
  # ---- E2S Listener .env (source=EVM, target=SVM) ----
...
EOF
  ...
  # ---- E2S Submitter .env (source=EVM, target=SVM) ----
...
# Save all PIDs
echo "$ALL_PIDS" | tr ' ' '\n' | grep -v '^$' > "$PID_DIR/relayers.pid"
log ""
log "All 9 processes started. PIDs saved to $PID_DIR/relayers.pid"
log "Logs in: $LOG_DIR/"
log ""
log "To stop: ./stop-relayers.sh"
log "To check health: curl http://localhost:8081/health"
