#!/usr/bin/env bash
# deploy-mainnet.sh
# One-shot: build + deploy + init + register relayers on Ethereum Mainnet + 1024chain Mainnet
set -euo pipefail
...
RELAYER_FUND_AMOUNT="${RELAYER_FUND_AMOUNT:-10000000000000000}"

log() { echo "[deploy-mainnet][$(date '+%H:%M:%S')] $*"; }
die() { echo "[deploy-mainnet] ERROR: $*" >&2; exit 1; }

BRIDGE_CFG=$(jq -r ".\"$BRIDGE_ID\"" "$BRIDGES_FILE")
[ "$BRIDGE_CFG" != "null" ] || die "Bridge $BRIDGE_ID not found in $BRIDGES_FILE"
...
LIQUIDITY=$(echo "$BRIDGE_CFG" | jq -r '.liquidity_amount // "0"')

ADMIN_EVM_KEY=$(jq -r '.private_key' "$ADMIN_EVM_KEY_FILE")
ADMIN_EVM_ADDR=$(jq -r '.address' "$ADMIN_EVM_KEY_FILE")

log "=========================================="
log "  MAINNET Deploy — $BRIDGE_ID"
log "=========================================="
log "EVM RPC:      $EVM_RPC"
log "EVM Chain ID: $EVM_CHAIN_ID"
log "EVM Token:    $EVM_TOKEN"
log "SVM RPC:      $SVM_RPC"
log "SVM Chain ID: $SVM_CHAIN_ID"
log "SVM Token:    $SVM_TOKEN"
log "Admin EVM:    $ADMIN_EVM_ADDR"
log "Liquidity:    $LIQUIDITY"
log ""

ETH_BAL=$(cast balance "$ADMIN_EVM_ADDR" --rpc-url "$EVM_RPC" --ether)
log "Admin ETH balance: $ETH_BAL"
...
PROGRAM_KEYPAIR="/tmp/bridge1024-mainnet-program-keypair.json"
...
# ============================================================
log ">>> Step 3: Deploying EVM contract to Ethereum Mainnet..."
cd "$PROJECT_ROOT/evm/bridge1024"
...
EVM_CONTRACT=$(echo "$DEPLOY_OUTPUT" | grep -oE 'Deployed to: 0x[0-9a-fA-F]+' | grep -oE '0x[0-9a-fA-F]+')
[ -n "$EVM_CONTRACT" ] || die "Failed to extract EVM contract address"
log "EVM contract deployed: $EVM_CONTRACT"

# ============================================================
# Step 4: Deploy SVM program
# ============================================================
log ">>> Step 4: Deploying SVM program to 1024chain Mainnet..."
...
RELAYER_MIN_BALANCE="$RELAYER_MIN_BALANCE" \
RELAYER_FUND_AMOUNT="$RELAYER_FUND_AMOUNT" \
