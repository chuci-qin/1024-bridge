...
op_evm_info() {
  ...
  local on_bridge_fee on_gasless_fee
  on_bridge_fee=$(evm_read "$rpc" "$bridge_addr" "bridgeFee()(uint64)" 2>/dev/null | xargs) || on_bridge_fee="0"
  on_gasless_fee=$(evm_read "$rpc" "$bridge_addr" "gaslessFee()(uint64)" 2>/dev/null | xargs) || on_gasless_fee="0"
  echo "    Bridge fee:     ${on_bridge_fee} ($(echo "scale=6; ${on_bridge_fee} / 1000000" | bc 2>/dev/null || echo "?") USDC)" >&2
  if [[ "$on_gasless_fee" == "0" ]]; then
    echo "    Gasless fee:    ${on_gasless_fee} (gasless deposit path DISABLED — stakeWithAuthorization reverts GaslessDisabled)" >&2
  else
    echo "    Gasless fee:    ${on_gasless_fee} ($(echo "scale=6; ${on_gasless_fee} / 1000000" | bc 2>/dev/null || echo "?") USDC)" >&2
  fi
